    ...
    def __init__(self, port:int = 5050) -> None:
        ...
        while port_not_found:
            try:
                ...
                server_logger.warning(f'Porta encontrada! Port = {self.port}')
                port_not_found = False
            except OSError as error:
                server_logger.warning(f'Erro: {error}. Porta padrão "{self.port}" ocupada. Buscando nova porta.')
                self.port += 1
                self.addr = (self.serverIp , self.port)
            
        server_logger.info(msg=f'Server bind to address: {self.serverIp}:{self.port}')
        self.connection:socket.socket = None
        
    def start_listen(self) -> None:   
        self.server.listen()
        server_logger.info(msg='Aguardando conexão ...')
        self.connection, address = self.server.accept()
        server_logger.info(msg=f'IP={':'.join(str(item) for item in address)} connected!')
        
    def send_message(self, message:str) -> None: 
        if message != "" or message is not None:
            message_enc: bytes = message.encode(ENCODING_SERIAL)
            self.connection.send(message_enc)
            server_logger.info(f'Sended message = {message}')
        else:
            server_logger.warning(f'Invalid message format! Message = {message}')
    
    def close_connection(self) -> None:
        server_logger.info(msg='Conexão encerrada!')
